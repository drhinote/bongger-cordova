...
function Capsule() {
    ...
    this.refreshBalance = () => {
        return new Promise((o, x) => {
            ...
            $http({ 
                url: 'https://api.greens.cards/unspent',
                method: 'POST',
                data: addresses
            }).then(res => {
                this.unspent = res.data;
                var save = false;
                angular.forEach(this.unspent, u => angular.forEach(u.unspent, v => { 
                    if(v.value && !this.data.transactions[v.txid]) {
                        this.data.transactions[v.txid] = { to: u.address, amount: v.value, fee: 0, value: 0, date: new Date().toISOString(v.time*1000), time: v.time, received: true  };
                        save = true;
                    }        
                }));
                if(save) this.save();
    ...
    this.getAllAddresses = () => {
        ...
        for(var i = 0; i < 64; i++) {
            var salt = i%64;
            var hash = sha256(this.pin + this.data.phrase + salt);
            var d = coins.bigi.fromBuffer(hash);
            var addr = new coins.bitcoin.ECPair(d, null, { network: coins.bongger });
            addresses.push(addr.getAddress());
            this.signatures[addr.getAddress()] = addr.toWIF();
        }
        return addresses;
    };
    this.send = (amount, value, address, newLabel, callback) => {
            var change = -(amount + 1);
            if(!this.data.transactions) this.data.transactions = {};
            var spending = [];
            for(var i = 0; i < this.unspent.length && change < 0; i++) {
                var keys = Object.keys(this.unspent[i].unspent);
                for(var j = 0; j < keys.length && change < 0; j++) {
                    change += parseFloat(this.unspent[i].unspent[keys[j]].value);
                    this.unspent[i].unspent[keys[j]].address = this.unspent[i].address;
                    spending.push(this.unspent[i].unspent[keys[j]]);
                }
            }
            if(change < 0) {
                ...
                var sigs = [];
                var tx = new coins.bitcoin.TransactionBuilder(coins.bongger);
                spending.forEach(unspent => {
                    tx.addInput(unspent.txid, unspent.vout);
                    sigs.push(this.signatures[unspent.address]);
                });
                tx.addOutput(address, Math.floor(amount*100000000));
                if(change > 0) {
                    tx.addOutput(this.getAddress().getAddress(), Math.floor(change*100000000));
                }
                sigs.forEach((sig, i) => tx.sign(i, coins.bitcoin.ECPair.fromWIF(sig, coins.bongger)));
                 var serialized = tx.build().toHex();
                 $http({
                        url: 'https://api.greens.cards/tx',
                        method: 'POST',
                        data: { tx: serialized }
                    }).then(res => {
                        var oldLabel = this.data.addresses[address];
                        if(newLabel) {
                            this.data.addresses[address] = newLabel;
                            delete this.data.labels[oldLabel];
                            this.data.labels[newLabel] = address;
                            oldLabel = newLabel;
                        }
                        this.data.transactions[res.data + 'out'] = { to: oldLabel?oldLabel:address, amount: amount, fee: 1, value: value, date: new Date().toISOString(), time: Math.floor(new Date().getTime()/1000)  };
                        if(change > 0) this.data.transactions[res.data] = { to: oldLabel?oldLabel:address, amount: amount, fee: 1, value: value, date: new Date().toISOString(), time: Math.floor(new Date().getTime()/1000), received: true, change: true };
                        this.save();
                        this.refreshBalance();
                        callback("Transaction successful");
                    }).catch(err => {
                        callback("Unable to broadcast transaction");
                    });
            }
        };
    this.balance = () => { 
        var bal = 0;
        this.unspent.forEach(u => angular.forEach(u.unspent, v => { if(!v.pending) bal += parseFloat(v.value); }));
        return bal;
    };
    this.pending = () => {
        var bal = 0;
    ...
    this.transactions = () => {
        var trans = [];
        angular.forEach(this.data.transactions, t => { if(!t.change) trans.push(t); });
        trans.sort((a, b) => {
    ...
    this.encrypted = localStorage.getItem('data');
    this.getAddress = () => {
        var salt = this.data.num = ((this.data.num || 0) + 1)%64;
        this.save();
        var hash = sha256(this.pin + this.data.phrase + salt);
        var d = coins.bigi.fromBuffer(hash);
        return new coins.bitcoin.ECPair(d, null, { network: coins.bongger });
    };
    this.reset = () => {
    ...
    this.initWallet = (newPin, phrase) => {
        this.pin = newPin;
        this.data = { success: true, phrase: (phrase || this.newPhrase()).trim().toLowerCase(), labels: {}, addresses: {} };
        this.save();
    };
    this.save = () => {
        if(this.pin) {
            localStorage.setItem('data', this.encrypted = CryptoJS.AES.encrypt(JSON.stringify(this.data), sha256(this.pin)));
        }
    };
    this.load = pin => {
        try {
            this.data = JSON.parse(CryptoJS.AES.decrypt(this.encrypted, sha256(pin)).toString(CryptoJS.enc.Utf8));
            this.pin = pin;
            return this.data;
        } catch(error) {
            console.log(error);
